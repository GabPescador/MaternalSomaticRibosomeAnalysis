...
# Author: Fengchao Yu
# Adapted by: Gabriel da Silva Pescador
...
# Accounts for .raw, .RAW, .wiff, .d and .mzML files inside root folder
for dir in "$root"/; do
  if [ -d "$dir" ]; then
    num_files=$(find "$dir" -type f -name "*.raw" -o -name "*.RAW" | wc -l)
    echo "Number of raw  files in $dir: $num_files .raw"
    
    num_files=$(find "$dir" -type f -name "*.wiff" | wc -l)
    echo "Number of wiff  files in $dir: $num_files .wiff"

    num_files=$(find "$dir" -type d -name "*.d" | wc -l)
    echo "Number of .d  files in $dir: $num_files .d"

    num_files=$(find "$dir" -type f -name "*.mzML" | wc -l)
    echo "Number of mzML files in $dir: $num_files .mzML"
  fi
done
