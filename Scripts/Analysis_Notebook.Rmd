...
A list of all datasets that passed quality check is included in "/ProcessingFiles/Datasets_metadata.xlsx".
...
for(i in df$Intersection){
  ...
  colnames(temp) <- c(i, "type")
  temp <- merge(temp, unique(Combined[,c(1,2)]),
        by.x=i, by.y="Accession")
...
for(i in df$Intersection){
  ...
  write_csv(temp, paste0(here("OutputTables/"), "piRNA_", i, ".csv"))
}
...
Here, we will get a list of the top 10 translated genes from our [proteomics dataset](https://doi.org/10.1016/j.celrep.2024.114769) for Leo to use.
...
Sequences were downloaded directly from UNIPROT.
...
Again, these sequences were manually downloaded from UNIPROT by searching for their species ID and GO term related to translation.
